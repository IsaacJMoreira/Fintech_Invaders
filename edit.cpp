...
#define PEGUE_PAGUE_KILL_HITS 2
...
struct GameScene : public Scene
{
    ...
    void update(int updateCount) override
    {
        ...
        if (peguePagueActive)
        {
            ...
            // --- AI movement calculation (UNCHANGED) ---
            ...
            // --- CLEAN TRACKING MOVEMENT ---

            // Smooth horizontal tracking
            float targetX = peguePagueX;
            float dx = targetX - peguePague->x;

            // control how fast it follows
            float followSpeed = 0.2; // tweak this (0.1 = slow, 0.3 = fast)

            peguePague->x += dx * followSpeed;

            // vertical movement (constant descent)
            peguePague->y += 1;
            ...
            // detect collisions ONCE per frame
