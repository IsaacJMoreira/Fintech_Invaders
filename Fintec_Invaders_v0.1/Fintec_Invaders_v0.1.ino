...
#define SCALE_X (SCREEN_WIDTH / BASE_WIDTH)
#define SCALE_Y (SCREEN_HEIGHT / BASE_HEIGHT)

#define PLAYER_SIZE 64
#define PLAYER_SPEED (4 * SCALE_X)
////////////////////////////////////////////////
...
struct IntroScene : public Scene {
  ...
  IntroScene()
    : Scene(0, 20, SCREEN_WIDTH, SCREEN_HEIGHT) {
  }
...
struct GameScene : public Scene {
  ...
  enum SpriteType { TYPE_PLAYERFIRE,
                    TYPE_ENEMIESFIRE,
                    TYPE_ENEMY,
                    TYPE_PLAYER,
                    TYPE_SHIELD,
  ...
  enum GameState { GAMESTATE_PLAYING,
  ...
  static const int ENEMIES_X_SPACE = 16 * SCALE_X;
  static const int ENEMIES_Y_SPACE = 10 * SCALE_Y;

  static const int ENEMIES_START_X =
    SCREEN_WIDTH / 2 - (ROWENEMIESCOUNT * ENEMIES_X_SPACE) / 2;

  static const int ENEMIES_START_Y = 80 * SCALE_Y;

  static const int ENEMIES_STEP_X = 6 * SCALE_X;
  static const int ENEMIES_STEP_Y = 8 * SCALE_Y;

  static const int PLAYER_Y = SCREEN_HEIGHT - PLAYER_SIZE - 20;  ///////////////////
  ...
  GameScene()
    : Scene(SPRITESCOUNT, 20, SCREEN_WIDTH, SCREEN_HEIGHT) {
  }
  ...
  void init() {
    ...
    player_->moveTo((SCREEN_WIDTH - PLAYER_SIZE) / 2, PLAYER_Y);  ///////////////////////
...
for (int i = 0; i < SHIELDSCOUNT; ++i) {
    int shieldSpacing = SCREEN_WIDTH / (SHIELDSCOUNT + 1);

    shields_[i].addBitmap(&bmpShield[i])->moveTo(shieldSpacing * (i + 1) - 40, PLAYER_Y - 120);

    // garante que não saia da tela
    shields_[i].y = iclamp(shields_[i].y, 0, SCREEN_HEIGHT - shields_[i].getHeight());
    ...
    canvas.drawTextFmt(SCREEN_WIDTH - 120, 20, "Level %02d", level_);
  ...
  void moveEnemy(SISprite* enemy, int x, int y, bool* touchSide) {
    if (enemy->visible) {
      if (x <= 0 || x >= getWidth() - enemy->getWidth())
        *touchSide = true;
      // Limitar Y e X do sprite antes de mover
      x = iclamp(x, 0, getWidth() - enemy->getWidth());
      y = iclamp(y, 0, getHeight() - enemy->getHeight());
      enemy->moveTo(x, y);
  ...
  void update(int updateCount) {
    ...
    if (gameState_ == GAMESTATE_PLAYING || gameState_ == GAMESTATE_PLAYERKILLED) {
      ...
      if ((updateCount % std::max(3, 21 - level_ * 2)) == 0) {
        ...
        // Limita Y para não ultrapassar a tela
        int maxEnemiesY = SCREEN_HEIGHT - 64 - 10;  // 64 = altura do maior inimigo, 10 = margem
        enemiesY_ += (enemiesDir_ >> 2 & 1) * ENEMIES_STEP_Y;
        enemiesY_ = iclamp(enemiesY_, 0, maxEnemiesY);  // <-- aqui
      ...
      if (enemiesFire_->visible) {
        ...
        if (enemiesFire_->y > SCREEN_HEIGHT - enemiesFire_->getHeight())
          enemiesFire_->visible = false;
        else
    ...
    if (gameState_ == GAMESTATE_GAMEOVER) {
      ...
      if ((IntroScene::controller_ == 1 && keyboard->isVKDown(fabgl::VK_SPACE)) || (IntroScene::controller_ == 2 && mouse->deltaAvailable() && mouse->getNextDelta(nullptr, 0) && mouse->status().buttons.left)) {
        stop();
        DisplayController.removeSprites();
      }
    }
  ...
  // player shoots
  void fire() {
    playerFire_->moveTo(player_->x + PLAYER_SIZE / 2, player_->y - 4)->visible = true;
  ...
  void showLives() {
    int hudY = SCREEN_HEIGHT - 30;

    canvas.fillRectangle(10, hudY, 200, SCREEN_HEIGHT - 10);

    canvas.setPenColor(Color::White);
    canvas.drawTextFmt(15, hudY + 2, "%d", lives_);

    for (int i = 0; i < lives_; ++i)
      canvas.drawBitmap(40 + i * (bmpPlayer.width + 5), hudY + 2, &bmpPlayer);
  }
...
int GameScene::level_ = 2;  //<--------- start level
...
void setup() {
  ...
  DisplayController.moveScreen(22, 0);
}
